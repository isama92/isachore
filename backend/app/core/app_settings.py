...
async def get_app_settings(session: AsyncSession) -> AppSettings:
    ...
    The migration seeds the row, but get-or-create keeps this robust for tests
    (which build the schema via create_all and never seed) and for any DB where
    the row is missing. The new row is flushed so its defaults are populated but
