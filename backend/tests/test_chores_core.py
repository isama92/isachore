...
def test_next_slot_daily_weekly_preserve_time() -> None:
    ...
    assert next_slot_after(base, DAILY) == datetime(2026, 7, 19, 14, 30, tzinfo=UTC)
    assert next_slot_after(base, WEEKLY) == datetime(2026, 7, 25, 14, 30, tzinfo=UTC)


def test_next_slot_applies_the_interval() -> None:
    base = datetime(2026, 7, 18, tzinfo=UTC)
    assert next_slot_after(base, repeating(RepeatPeriod.daily, 3)) == datetime(
        2026, 7, 21, tzinfo=UTC
    )
    assert next_slot_after(base, repeating(RepeatPeriod.weekly, 2)) == datetime(
        2026, 8, 1, tzinfo=UTC
    )
...
def test_next_slot_monthly_clamps_day_and_rolls_year() -> None:
    ...
    assert next_slot_after(datetime(2026, 1, 31, 9, 0, tzinfo=UTC), MONTHLY) == datetime(
        2026, 2, 28, 9, 0, tzinfo=UTC
    )
    # December rolls into the next year.
    assert next_slot_after(datetime(2026, 12, 15, tzinfo=UTC), MONTHLY) == datetime(
        2027, 1, 15, tzinfo=UTC
    )
    # An interval that clears February keeps the 31st.
    assert next_slot_after(
        datetime(2026, 1, 31, tzinfo=UTC), repeating(RepeatPeriod.monthly, 2)
...
def test_next_slot_yearly_clamps_leap_day() -> None:
    ...
    assert next_slot_after(leap_day, YEARLY) == datetime(2029, 2, 28, tzinfo=UTC)
    # + 2 years lands on a non-leap year too, so it still clamps.
    assert next_slot_after(leap_day, repeating(RepeatPeriod.yearly, 2)) == datetime(
        2030, 2, 28, tzinfo=UTC
    )
    # + 4 years lands on a leap year, so the 29th survives.
    assert next_slot_after(leap_day, repeating(RepeatPeriod.yearly, 4)) == datetime(
...
def test_next_slot_manual_raises() -> None:
    with pytest.raises(ValueError, match="no recurrence interval"):
        next_slot_after(datetime(2026, 7, 18, tzinfo=UTC), MANUAL)
...
def test_next_slot_single_weekday_is_a_whole_week_on() -> None:
    ...
    assert next_slot_after(datetime(2026, 7, 21, tzinfo=UTC), weekly_on(TUE)) == datetime(
...
def test_next_slot_hops_between_weekdays_within_the_week() -> None:
    ...
    assert next_slot_after(datetime(2026, 7, 21, tzinfo=UTC), rule) == datetime(
        2026, 7, 24, tzinfo=UTC
    )
    assert next_slot_after(datetime(2026, 7, 24, tzinfo=UTC), rule) == datetime(
        2026, 7, 28, tzinfo=UTC
    )
    # Sun 19 is past both selected days, so it wraps to the next week's Tuesday.
    assert next_slot_after(datetime(2026, 7, 19, tzinfo=UTC), rule) == datetime(
        2026, 7, 21, tzinfo=UTC
    )
...
def test_next_slot_from_an_unselected_weekday_snaps_onto_the_grid() -> None:
    ...
    assert next_slot_after(datetime(2026, 7, 22, tzinfo=UTC), weekly_on(MON)) == datetime(
...
def test_next_slot_weekdays_can_skip_the_weekend() -> None:
    ...
    assert next_slot_after(datetime(2026, 7, 24, tzinfo=UTC), weekdays_only) == datetime(
        2026, 7, 27, tzinfo=UTC
    )
...
def test_next_slot_adjacent_weekdays_are_one_day_apart() -> None:
    ...
    assert next_slot_after(datetime(2026, 7, 18, tzinfo=UTC), weekly_on(SAT, SUN)) == datetime(
        2026, 7, 19, tzinfo=UTC
    )
    ...
    assert next_slot_after(datetime(2026, 7, 19, tzinfo=UTC), weekly_on(MON)) == datetime(
...
def test_next_slot_interval_is_spent_only_on_crossing_the_week() -> None:
    ...
    assert next_slot_after(datetime(2026, 7, 21, tzinfo=UTC), fortnightly) == datetime(
        2026, 7, 24, tzinfo=UTC
    )  # +3, same week
    assert next_slot_after(datetime(2026, 7, 24, tzinfo=UTC), fortnightly) == datetime(
        2026, 8, 4, tzinfo=UTC
    )  # +11, two weeks on
    # A single weekday has no intra-week hop, so it always spends the interval.
    assert next_slot_after(datetime(2026, 7, 20, tzinfo=UTC), weekly_on(MON, every=2)) == datetime(
        2026, 8, 3, tzinfo=UTC
    )
...
def test_next_slot_weekday_hop_preserves_time_of_day() -> None:
    assert next_slot_after(
        datetime(2026, 7, 21, 9, 15, tzinfo=UTC), weekly_on(TUE, FRI)
...
def test_snap_to_slot_leaves_a_valid_slot_alone_and_is_idempotent() -> None:
    ...
    assert snap_to_slot(tuesday, rule) == tuesday
    assert snap_to_slot(snap_to_slot(tuesday, rule), rule) == tuesday
...
def test_snap_to_slot_moves_forward_to_the_nearest_selected_weekday() -> None:
    ...
    assert snap_to_slot(datetime(2026, 7, 22, tzinfo=UTC), weekly_on(TUE, FRI)) == datetime(
        2026, 7, 24, tzinfo=UTC
    )
...
def test_snap_to_slot_never_spends_the_interval() -> None:
    ...
    assert snap_to_slot(datetime(2026, 7, 18, tzinfo=UTC), weekly_on(TUE, every=4)) == datetime(
        2026, 7, 21, tzinfo=UTC
    )
...
def test_snap_to_slot_is_identity_for_unpinned_rules() -> None:
    ...
    for rule in (DAILY, WEEKLY, MONTHLY, YEARLY, MANUAL, repeating(RepeatPeriod.weekly, 3)):
        assert snap_to_slot(wednesday, rule) == wednesday
...
def test_advance_anchor_early_completion_advances_one_interval() -> None:
    ...
    assert next_slot_after(anchor, DAILY) == datetime(2026, 7, 22, tzinfo=UTC)
...
def test_advance_anchor_on_time_completion_advances_to_next_day() -> None:
    ...
    assert next_slot_after(anchor, DAILY) == datetime(2026, 7, 21, tzinfo=UTC)
...
def test_advance_anchor_weekly_overdue_preserves_weekday() -> None:
    ...
    assert next_slot_after(anchor, WEEKLY) == datetime(2026, 7, 20, tzinfo=UTC)
...
def test_advance_anchor_monthly_overdue_stays_on_day_of_month() -> None:
    ...
    assert next_slot_after(anchor, MONTHLY) == datetime(2026, 3, 15, tzinfo=UTC)
...
def test_advance_anchor_keeps_the_cycle_phase_over_a_long_gap() -> None:
    ...
    assert next_slot_after(anchor, weekly_on(TUE, every=2)) == datetime(2026, 8, 18, tzinfo=UTC)
...
def test_first_occurrence_is_start_date_midnight_utc() -> None:
    assert first_occurrence(date(2026, 7, 10), WEEKLY) == datetime(2026, 7, 10, tzinfo=UTC)
    assert first_occurrence(date(2026, 7, 10), MONTHLY) == datetime(2026, 7, 10, tzinfo=UTC)
...
def test_first_occurrence_snaps_forward_to_a_selected_weekday() -> None:
    ...
    assert first_occurrence(date(2026, 7, 20), rule) == datetime(2026, 7, 21, tzinfo=UTC)
    # Wed 22 -> Fri 24: forward only, never back to Tuesday.
    assert first_occurrence(date(2026, 7, 22), rule) == datetime(2026, 7, 24, tzinfo=UTC)
    # Fri 24 already qualifies, so it is kept.
    assert first_occurrence(date(2026, 7, 24), rule) == datetime(2026, 7, 24, tzinfo=UTC)
...
def test_first_occurrence_snap_does_not_wait_out_the_interval() -> None:
    ...
    assert first_occurrence(date(2026, 7, 18), weekly_on(TUE, every=2)) == datetime(
        2026, 7, 21, tzinfo=UTC
    )
...
def test_next_occurrence_after_weekly_preserves_weekday() -> None:
    ...
    nxt = next_occurrence_after(scheduled, datetime(2026, 7, 15, 10, 0, tzinfo=UTC), WEEKLY)
...
def test_next_occurrence_after_twice_a_week_alternates() -> None:
    ...
    assert (
        next_occurrence_after(
            datetime(2026, 7, 21, tzinfo=UTC), datetime(2026, 7, 21, 19, 0, tzinfo=UTC), rule
        )
        == friday
    )
    assert next_occurrence_after(
        friday, datetime(2026, 7, 24, 19, 0, tzinfo=UTC), rule
...
def test_next_occurrence_after_fortnightly_two_weekdays() -> None:
    ...
    assert next_occurrence_after(
        datetime(2026, 7, 21, tzinfo=UTC), datetime(2026, 7, 21, 9, 0, tzinfo=UTC), rule
    ) == datetime(2026, 7, 24, tzinfo=UTC)
    assert next_occurrence_after(
        datetime(2026, 7, 24, tzinfo=UTC), datetime(2026, 7, 24, 9, 0, tzinfo=UTC), rule
...
def test_next_occurrence_after_off_grid_row_self_heals() -> None:
    ...
    assert snap_to_slot(datetime(2026, 7, 22, tzinfo=UTC), weekly_on(TUE, every=2)) == datetime(
        2026, 7, 28, tzinfo=UTC
    )
